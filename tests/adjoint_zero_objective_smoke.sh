...
case_dir="output-adjoint-zero-objective-smoke"
if [[ ! -d "${case_dir}" ]]; then
  case_dir="output-adjoint_zero_objective_smoke"
fi

kernel_file="${case_dir}/adjoint_kernels_rank_00000.txt"

echo ""
echo "Adjoint zero-objective smoke summary:"
if [[ -f "${kernel_file}" ]]; then
  echo "kernel file: present"
  awk -F '	' '
    $1 == "# objective_value" {print "objective", $2, $3}
    $1 == "# contribution_integral" {print "kernel_integral", $2"|" $3"|" $4, $5}
...
echo ""
echo "Adjoint zero-objective control gradient summary:"

...
# of the velocity-norm volume-term diagnostic produced by the adjoint postprocessor.

perl "$(dirname "$0")/cmake/default" "$1"

check_file="output-adjoint-volume-term-velocity-norm-smoke/adjoint_finite_difference_checks_rank_00000.txt"
if [[ ! -f "${check_file}" ]]; then
  check_file="output-adjoint_volume_term_velocity_norm_smoke/adjoint_finite_difference_checks_rank_00000.txt"
fi

echo ""
echo "Adjoint velocity-norm volume-term benchmark summary:"
if [[ -f "${check_file}" ]]; then
